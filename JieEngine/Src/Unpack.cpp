...
SDL_Surface * UnpackData::Image(string FileName){
...
Mix_Chunk * UnpackData::Mixer(string FileName){
    if(this->_Data.find(FileName) != this->_Data.end()){
...
TTF_Font * UnpackData::Font(string FileName,int FontSize){
    if(this->_Data.find(FileName) != this->_Data.end()){
...
UnpackData JieEngine::Unpack(string FileUrl){
    UnpackData Ret;
    ifstream File;
